...
type aggKey struct{ Customer, Project, Activity string; Billable bool }
...
var reportCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(entries) == 0 { fmt.Println("No entries."); return }
		r := getRounding()

		by := strings.Split(repBy, ",")
		useBy := map[string]bool{}
		for _, f := range by { if f != "" { useBy[strings.TrimSpace(f)] = true } }
		...
		for _, e := range entries {
			...
			k := aggKey{}
			if useBy["customer"] { k.Customer = e.Customer }
			if useBy["project"] { k.Project = e.Project }
			if useBy["activity"] { k.Activity = e.Activity }
			if useBy["billable"] { k.Billable = e.Billable }
			if _, ok := agg[k]; !ok { agg[k] = &aggVal{} }
			agg[k].RawMin += min
		...
		sort.Slice(keys, func(i, j int) bool {
			...
			if keys[i].Customer != keys[j].Customer { return keys[i].Customer < keys[j].Customer }
			if keys[i].Project != keys[j].Project { return keys[i].Project < keys[j].Project }
			return keys[i].Activity < keys[j].Activity
		})
		for _, k := range keys {
			v := agg[k]
			fmt.Printf("Group:")
			if useBy["customer"] { fmt.Printf(" customer=%s", k.Customer) }
			if useBy["project"] { fmt.Printf(", project=%s", k.Project) }
			if useBy["activity"] { fmt.Printf(", activity=%s", k.Activity) }
			if useBy["billable"] { fmt.Printf(", billable=%v", k.Billable) }
